...
import { NINVerifyWithPhoneFormData } from "../../types";
import { ninVerifyWithPhoneFormSchema } from "../../schemas";
import { useAppContext } from "../../contexts/AppContext";
import { ninSlips } from "../../constants";
import { verifyNINWithPhone } from "../../api/verification";
...
const NINWithPhone = () => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<NINVerifyWithPhoneFormData>({
    resolver: zodResolver(ninVerifyWithPhoneFormSchema),
  });
  const { showToast } = useAppContext();

  const [errorMessage, setErrorMessage] = useState("");

  const formNames = ["slipType", "phoneNumber"];
  ...
  const { mutate, isPending: verifyingNIN } = useMutation({
    mutationFn: verifyNINWithPhone,
  });

  const onSubmit = handleSubmit((data: NINVerifyWithPhoneFormData) => {
